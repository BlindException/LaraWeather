<div>
    ...
    <section aria-label="{{ (" Current Conditions") }}">
        ...
        <div>
            <p>Wind:</p>
            <p>{{ (round($current["wind_mph"])." mph ".$current["wind_dir"]) }}</p>
        </div>
    </section>
    <section aria-label="{{ (" 5 Day Forecast") }}">
        ...
        <div>
            ...
            <p>{{ ($forecastDay->day->condition->text) }}</p><img href="{{ ($forecastDay->day->condition->icon)}}"
                alt="{{ ($forecastDay->day->condition->text)}}">

            <p>{{ ($forecastDay->day->daily_chance_of_rain."% rain/".$forecastDay->day->daily_chance_of_snow."% snow")
                }}</p>
